    ...
    <v-col cols="12">
      <v-data-table
        ...
        <v-icon
          ...
          class="me-2"
          color="warning"
          @click="toggleBan(item.id, false)"
        >
          mdi-lock-open-variant
          <v-tooltip activator="parent" location="top">{{ $t('actions.unban') }}</v-tooltip>
        </v-icon>
        <v-menu
          v-else
          v-model="banOverlay[item.id]"
        ...
        >
          <template v-slot:activator="{ props }">
            <v-icon class="me-2" color="error" v-bind="props">
              mdi-lock
              <v-tooltip activator="parent" location="top">{{ $t('actions.ban') }}</v-tooltip>
            </v-icon>
          </template>
          <v-card :title="$t('actions.ban')" rounded="lg">
            <v-divider></v-divider>
            <v-card-text>{{ $t('confirm') }}</v-card-text>
            <v-card-actions>
              <v-btn color="error" variant="outlined" @click="toggleBan(item.id, true)">{{ $t('yes') }}</v-btn>
              <v-btn color="success" variant="outlined" @click="banOverlay[item.id] = false">{{ $t('no') }}</v-btn>
            </v-card-actions>
          </v-card>
        </v-menu>
        <v-menu
...
const banOverlay = ref<Record<number, boolean>>({})

const toggleBan = async (id: number, banned: boolean) => {
  if (!clients.value.some(c => c.id === id)) return
  const success = await Data().banClient(id, banned)
  if (success) banOverlay.value[id] = false
}
