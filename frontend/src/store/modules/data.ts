...
const Data = defineStore('Data', {
  ...
  actions: {
    ...
    // Ban/unban a client. Immediate action (no Save round-trip) since it must
    // take effect at once for a security-sensitive operation. Form-encoded so
    // the backend's FormValue("id"/"banned") reads it; the response carries the
    // refreshed clients+inbounds, applied via setNewData.
    async banClient (id: number, banned: boolean): Promise<boolean> {
      const postData = new URLSearchParams()
      postData.append('id', id.toString())
      postData.append('banned', banned.toString())
      const msg = await HttpUtils.post('api/ban', postData)
      if (msg.success) {
        push.success({
          title: i18n.global.t('success'),
          duration: 5000,
          message: i18n.global.t(banned ? 'actions.ban' : 'actions.unban')
        })
        this.setNewData(msg.obj)
      }
      return msg.success
