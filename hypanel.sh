...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/by-sonic/HyPanel/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/by-sonic/HyPanel/master/install.sh"
...
uninstall() {
    ...
    systemctl disable hypanel
    rm /etc/systemd/system/hypanel.service -f
    systemctl daemon-reload
    systemctl reset-failed
    rm /etc/hypanel/ -rf
    rm /usr/local/hypanel/ -rf

    echo ""
    echo -e "Uninstalled Successfully, If you want to remove this script, then after exiting the script run ${green}rm /usr/local/hypanel -f${plain} to delete it."
    echo ""
...
reset_admin() {
    ...
    if [[ $? == 0 ]]; then
        /usr/local/hypanel/hypanel admin -reset
    fi
    before_show_menu
}
...
reset_setting() {
    ...
    if [[ $? == 0 ]]; then
        /usr/local/hypanel/hypanel setting -reset
    fi
    before_show_menu
}
...
view_setting() {
    /usr/local/hypanel/hypanel setting -show
    view_uri
    before_show_menu
}

view_uri() {
    info=$(/usr/local/hypanel/hypanel uri)
    if [[ $? != 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/hypanel -N --no-check-certificate https://github.com/by-sonic/HyPanel/raw/main/hypanel.sh
    if [[ $? != 0 ]]; then
        echo ""
        LOGE "Failed to download script, Please check whether the machine can connect Github"
        before_show_menu
    else
        chmod +x /usr/bin/hypanel
...
check_hypanel_status() {
    count=$(ps -ef | grep "hypanel" | grep -v "grep" | wc -l)
    if [[ count -ne 0 ]]; then
        return 0
    else
        return 1
...
show_hypanel_status() {
    check_hypanel_status
    if [[ $? == 0 ]]; then
        echo -e "hypanel state: ${green}Running${plain}"
    else
        echo -e "hypanel state: ${red}Not Running${plain}"
    fi
}
...
show_usage() {
    echo -e "HyPanel Control Menu Usage"
    echo -e "------------------------------------------"
    echo -e "SUBCOMMANDS:" 
    echo -e "hypanel              - Admin Management Script"
    echo -e "hypanel start        - Start hypanel"
    echo -e "hypanel stop         - Stop hypanel"
    echo -e "hypanel restart      - Restart hypanel"
    echo -e "hypanel status       - Current Status of hypanel"
    echo -e "hypanel enable       - Enable Autostart on OS Startup"
    echo -e "hypanel disable      - Disable Autostart on OS Startup"
    echo -e "hypanel log          - Check hypanel Logs"
    echo -e "hypanel update       - Update"
    echo -e "hypanel install      - Install"
    echo -e "hypanel uninstall    - Uninstall"
    echo -e "hypanel help         - Control Menu Usage"
    echo -e "------------------------------------------"
}

show_menu() {
  echo -e "
  ${green}HyPanel Admin Management Script ${plain}
————————————————————————————————
  ${green}0.${plain} Exit
————————————————————————————————
...
————————————————————————————————
  ${green}11.${plain} HyPanel Start
  ${green}12.${plain} HyPanel Stop
  ${green}13.${plain} HyPanel Restart
  ${green}14.${plain} HyPanel Check State
  ${green}15.${plain} HyPanel Check Logs
  ${green}16.${plain} HyPanel Enable Autostart
  ${green}17.${plain} HyPanel Disable Autostart
————————————————————————————————
...
————————————————————————————————
 "
    ...
    10)
        ...
        ;;
    11)
        check_install && start hypanel
        ;;
    12)
        check_install && stop hypanel
        ;;
    13)
        check_install && restart hypanel
        ;;
    14)
        check_install && status hypanel
        ;;
    15)
        check_install && show_log hypanel
        ;;
    16)
        check_install && enable hypanel
        ;;
    17)
        check_install && disable hypanel
        ;;
    18)
        bbr_menu
        ;;
...
if [[ $# > 0 ]]; then
    case $1 in
    "start")
        check_install 0 && start hypanel 0
        ;;
    "stop")
        check_install 0 && stop hypanel 0
        ;;
    "restart")
        check_install 0 && restart hypanel 0
        ;;
    "status")
        check_install 0 && status 0
        ;;
    "enable")
        check_install 0 && enable hypanel 0
        ;;
    "disable")
        check_install 0 && disable hypanel 0
        ;;
    "log")
        check_install 0 && show_log hypanel 0
        ;;
    "update")
